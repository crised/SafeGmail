...
var mouseMotionCollect=0;var oldMoveHandler;function mouseMotionEntropy(maxsamp){if(mouseMotionCollect<=0){mouseMotionCollect=maxsamp;if((document.implementation.hasFeature("Events","2.0"))&&document.addEventListener){document.addEventListener("mousemove",mouseMoveEntropy,false);}else{if(document.attachEvent){document.attachEvent("onmousemove",mouseMoveEntropy);}else{oldMoveHandler=document.onmousemove;document.onmousemove=mouseMoveEntropy;}}}}
var mouseEntropyTime=0;function mouseMoveEntropy(e){if(!e){e=window.event;}
if(mouseMotionCollect>0){if(mouseEntropyTime--<=0){addEntropyByte(e.screenX&0xFF);addEntropyByte(e.screenY&0xFF);ce();mouseMotionCollect--;mouseEntropyTime=(entropyData[edlen-3]^entropyData[edlen-2]^entropyData[edlen-1])%19;}
...
function keyFromEntropy(){var i,k=new Array(32);if(edlen==0){alert("Blooie!  Entropy vector void at call to keyFromEntropy.");}
md5_init();for(i=0;i<edlen;i+=2){md5_update(entropyData[i]);}
md5_finish();for(i=0;i<16;i++){k[i]=digestBits[i];}
md5_init();for(i=1;i<edlen;i+=2){md5_update(entropyData[i]);}
md5_finish();for(i=0;i<16;i++){k[i+16]=digestBits[i];}
return k;}
var loadTime=(new Date()).getTime();var key;var prng;function setKey(){if(document.key.keytype[0].checked){var s=encode_utf8(document.key.text.value);var i,kmd5e,kmd5o;if(s.length==1){s+=s;}
md5_init();for(i=0;i<s.length;i+=2){md5_update(s.charCodeAt(i));}
md5_finish();kmd5e=byteArrayToHex(digestBits);md5_init();for(i=1;i<s.length;i+=2){md5_update(s.charCodeAt(i));}
md5_finish();kmd5o=byteArrayToHex(digestBits);var hs=kmd5e+kmd5o;key=hexToByteArray(hs);hs=byteArrayToHex(key);}else{var s=document.key.text.value;var hexDigits="0123456789abcdefABCDEF";var hs="",i,bogus=false;for(i=0;i<s.length;i++){var c=s.charAt(i);if(hexDigits.indexOf(c)>=0){hs+=c;}else{bogus=true;}}
if(bogus){alert("Error: Improper character(s) in hexadecimal key.");}
if(hs.length>(keySizeInBits/4)){alert("Warning: hexadecimal key exceeds "+
(keySizeInBits/4)+" digit maximum; truncated.");document.key.text.value=hs=hs.slice(0,64);}else{while(hs.length<(keySizeInBits/4)){hs+="0";}}
key=hexToByteArray(hs);}}
function Generate_key(){var i,j,k="";var i,j,k="";addEntropyTime();var seed=keyFromEntropy();var prng=new AESprng(seed);var hexDigits="0123456789ABCDEF";for(i=0;i<64;i++){k+=hexDigits.charAt(prng.nextInt(15));}
delete prng;return k;}
function Encrypt_text(){var v,i;var prefix="#####  Encrypted: decrypt with http://www.fourmilab.ch/javascrypt/\n",suffix="#####  End encrypted message\n";if(document.key.text.value.length==0){alert("Please specify a key with which to encrypt the message.");return;}
if(document.plain.text.value.length==0){alert("No plain text to encrypt!  Please enter or paste plain text in the field above.");return;}
document.cipher.text.value="";setKey();addEntropyTime();prng=new AESprng(keyFromEntropy());var plaintext=encode_utf8(document.plain.text.value);md5_init();for(i=0;i<plaintext.length;i++){md5_update(plaintext.charCodeAt(i));}
md5_finish();var header="";for(i=0;i<digestBits.length;i++){header+=String.fromCharCode(digestBits[i]);}
i=plaintext.length;header+=String.fromCharCode(i>>>24);header+=String.fromCharCode(i>>>16);header+=String.fromCharCode(i>>>8);header+=String.fromCharCode(i&0xFF);var ct=rijndaelEncrypt(header+plaintext,key,"CBC");if(document.plain.encoding[0].checked){v=armour_codegroup(ct);}else if(document.plain.encoding[1].checked){v=armour_hex(ct);}else if(document.plain.encoding[2].checked){v=armour_base64(ct);}
document.cipher.text.value=prefix+v+suffix;delete prng;}
function determineArmourType(s){var kt,pcg,phex,pb64,pmin;pcg=s.indexOf(codegroupSentinel);phex=s.indexOf(hexSentinel);pb64=s.indexOf(base64sent);if(pcg==-1){pcg=s.length;}
if(phex==-1){phex=s.length;}
if(pb64==-1){pb64=s.length;}
pmin=Math.min(pcg,Math.min(phex,pb64));if(pmin<s.length){if(pmin==pcg){kt=0;}else if(pmin==phex){kt=1;}else{kt=2;}}else{if(document.plain.encoding[0].checked){kt=0;}else if(document.plain.encoding[1].checked){kt=1;}else if(document.plain.encoding[2].checked){kt=2;}}
return kt;}
function Decrypt_text(){if(document.key.text.value.length==0){alert("Please specify a key with which to decrypt the message.");return;}
if(document.cipher.text.value.length==0){alert("No cipher text to decrypt!  Please enter or paste cipher text in the field above.");return;}
document.plain.text.value="";setKey();var ct=new Array(),kt;kt=determineArmourType(document.cipher.text.value);if(kt==0){ct=disarm_codegroup(document.cipher.text.value);}else if(kt==1){ct=disarm_hex(document.cipher.text.value);}else if(kt==2){ct=disarm_base64(document.cipher.text.value);}
var result=rijndaelDecrypt(ct,key,"CBC");var header=result.slice(0,20);result=result.slice(20);var dl=(header[16]<<24)|(header[17]<<16)|(header[18]<<8)|header[19];if((dl<0)||(dl>result.length)){alert("Message (length "+result.length+") truncated.  "+
dl+" characters expected.");dl=result.length;}
var i,plaintext="";md5_init();for(i=0;i<dl;i++){plaintext+=String.fromCharCode(result[i]);md5_update(result[i]);}
md5_finish();for(i=0;i<digestBits.length;i++){if(digestBits[i]!=header[i]){alert("Message corrupted.  Checksum of decrypted message does not match.");break;}}
document.plain.text.value=decode_utf8(plaintext);}
function array(n){for(i=0;i<n;i++){this[i]=0;}
this.length=n;}
function integer(n){return n%(0xffffffff+1);}
function shr(a,b){a=integer(a);b=integer(b);if(a-0x80000000>=0){a=a%0x80000000;a>>=b;a+=0x40000000>>(b-1);}else{a>>=b;}
return a;}
function shl1(a){a=a%0x80000000;if(a&0x40000000==0x40000000){a-=0x40000000;a*=2;a+=0x80000000;}else{a*=2;}
return a;}
function shl(a,b){a=integer(a);b=integer(b);for(var i=0;i<b;i++){a=shl1(a);}
return a;}
...
function md5_init(){count[0]=count[1]=0;state[0]=0x67452301;state[1]=0xefcdab89;state[2]=0x98badcfe;state[3]=0x10325476;for(i=0;i<digestBits.length;i++){digestBits[i]=0;}}
function md5_update(b){var index,i;index=and(shr(count[0],3),0x3F);if(count[0]<0xFFFFFFFF-7){count[0]+=8;}else{count[1]++;count[0]-=0xFFFFFFFF+1;count[0]+=8;}
buffer[index]=and(b,0xff);if(index>=63){transform(buffer,0);}}
function md5_finish(){var bits=new array(8);var padding;var i=0,index=0,padLen=0;for(i=0;i<4;i++){bits[i]=and(shr(count[0],(i*8)),0xFF);}
for(i=0;i<4;i++){bits[i+4]=and(shr(count[1],(i*8)),0xFF);}
index=and(shr(count[0],3),0x3F);padLen=(index<56)?(56-index):(120-index);padding=new array(64);padding[0]=0x80;for(i=0;i<padLen;i++){md5_update(padding[i]);}
...
return result;}
function formatPlaintext(plaintext){var bpb=blockSizeInBits/8;var i;if((!((typeof plaintext=="object")&&((typeof(plaintext[0]))=="number")))&&((typeof plaintext=="string")||plaintext.indexOf)){plaintext=plaintext.split("");for(i=0;i<plaintext.length;i++)
plaintext[i]=plaintext[i].charCodeAt(0)&0xFF;}
i=plaintext.length%bpb;if(i>0){plaintext=plaintext.concat(getRandomBytes(bpb-i));}
return plaintext;}
function getRandomBytes(howMany){var i,bytes=new Array();for(i=0;i<howMany;i++){bytes[i]=prng.nextInt(255);}
return bytes;}
...
return;if(mode=="CBC"){ct=getRandomBytes(bpb);}else{mode="ECB";ct=new Array();}
plaintext=formatPlaintext(plaintext);expandedKey=keyExpansion(key);for(var block=0;block<plaintext.length/bpb;block++){aBlock=plaintext.slice(block*bpb,(block+1)*bpb);if(mode=="CBC"){for(var i=0;i<bpb;i++){aBlock[i]^=ct[(block*bpb)+i];}}
...
return;if(key.length*8!=keySizeInBits)
return;if(!mode){mode="ECB";}
expandedKey=keyExpansion(key);for(block=(ciphertext.length/bpb)-1;block>0;block--){aBlock=decrypt(ciphertext.slice(block*bpb,(block+1)*bpb),expandedKey);if(mode=="CBC")
for(var i=0;i<bpb;i++)
pt[(block-1)*bpb+i]=aBlock[i]^ciphertext[(block-1)*bpb+i];else
pt=aBlock.concat(pt);}
if(mode=="ECB")
pt=decrypt(ciphertext.slice(0,bpb),expandedKey).concat(pt);return pt;}
(function(a,b,c){function g(a,c){var d=b.createElement(a||"div"),e;for(e in c)d[e]=c[e];return d}function h(a){for(var b=1,c=arguments.length;b<c;b++)a.appendChild(arguments[b]);return a}function j(a,b,c,d){var g=["opacity",b,~~(a*100),c,d].join("-"),h=.01+c/d*100,j=Math.max(1-(1-a)/b*(100-h),a),k=f.substring(0,f.indexOf("Animation")).toLowerCase(),l=k&&"-"+k+"-"||"";return e[g]||(i.insertRule("@"+l+"keyframes "+g+"{"+"0%{opacity:"+j+"}"+h+"%{opacity:"+a+"}"+(h+.01)+"%{opacity:1}"+(h+b)%100+"%{opacity:"+a+"}"+"100%{opacity:"+j+"}"+"}",0),e[g]=1),g}function k(a,b){var e=a.style,f,g;if(e[b]!==c)return b;b=b.charAt(0).toUpperCase()+b.slice(1);for(g=0;g<d.length;g++){f=d[g]+b;if(e[f]!==c)return f}}function l(a,b){for(var c in b)a.style[k(a,c)||c]=b[c];return a}function m(a){for(var b=1;b<arguments.length;b++){var d=arguments[b];for(var e in d)a[e]===c&&(a[e]=d[e])}return a}function n(a){var b={x:a.offsetLeft,y:a.offsetTop};while(a=a.offsetParent)b.x+=a.offsetLeft,b.y+=a.offsetTop;return b}var d=["webkit","Moz","ms","O"],e={},f,i=function(){var a=g("style");return h(b.getElementsByTagName("head")[0],a),a.sheet||a.styleSheet}(),o={lines:12,length:7,width:5,radius:10,rotate:0,color:"#000",speed:1,trail:100,opacity:.25,fps:20,zIndex:2e9,className:"spinner",top:"auto",left:"auto"},p=function q(a){if(!this.spin)return new q(a);this.opts=m(a||{},q.defaults,o)};p.defaults={},m(p.prototype,{spin:function(a){this.stop();var b=this,c=b.opts,d=b.el=l(g(0,{className:c.className}),{position:"relative",zIndex:c.zIndex}),e=c.radius+c.length+c.width,h,i;a&&(a.insertBefore(d,a.firstChild||null),i=n(a),h=n(d),l(d,{left:(c.left=="auto"?i.x-h.x+(a.offsetWidth>>1):c.left+e)+"px",top:(c.top=="auto"?i.y-h.y+(a.offsetHeight>>1):c.top+e)+"px"})),d.setAttribute("aria-role","progressbar"),b.lines(d,b.opts);if(!f){var j=0,k=c.fps,m=k/c.speed,o=(1-c.opacity)/(m*c.trail/100),p=m/c.lines;!function q(){j++;for(var a=c.lines;a;a--){var e=Math.max(1-(j+a*p)%m*o,c.opacity);b.opacity(d,c.lines-a,e,c)}b.timeout=b.el&&setTimeout(q,~~(1e3/k))}()}return b},stop:function(){var a=this.el;return a&&(clearTimeout(this.timeout),a.parentNode&&a.parentNode.removeChild(a),this.el=c),this},lines:function(a,b){function e(a,d){return l(g(),{position:"absolute",width:b.length+b.width+"px",height:b.width+"px",background:a,boxShadow:d,transformOrigin:"left",transform:"rotate("+~~(360/b.lines*c+b.rotate)+"deg) translate("+b.radius+"px"+",0)",borderRadius:(b.width>>1)+"px"})}var c=0,d;for(;c<b.lines;c++)d=l(g(),{position:"absolute",top:1+~(b.width/2)+"px",transform:b.hwaccel?"translate3d(0,0,0)":"",opacity:b.opacity,animation:f&&j(b.opacity,b.trail,c,b.lines)+" "+1/b.speed+"s linear infinite"}),b.shadow&&h(d,l(e("#000","0 0 4px #000"),{top:"2px"})),h(a,h(d,e(b.color,"0 0 1px rgba(0,0,0,.1)")));return a},opacity:function(a,b,c){b<a.childNodes.length&&(a.childNodes[b].style.opacity=c)}}),!function(){function a(a,b){return g("<"+a+' xmlns="urn:schemas-microsoft.com:vml" class="spin-vml">',b)}var b=l(g("group"),{behavior:"url(#default#VML)"});!k(b,"transform")&&b.adj?(i.addRule(".spin-vml","behavior:url(#default#VML)"),p.prototype.lines=function(b,c){function f(){return l(a("group",{coordsize:e+" "+e,coordorigin:-d+" "+-d}),{width:e,height:e})}function k(b,e,g){h(i,h(l(f(),{rotation:360/c.lines*b+"deg",left:~~e}),h(l(a("roundrect",{arcsize:1}),{width:d,height:c.width,left:c.radius,top:-c.width>>1,filter:g}),a("fill",{color:c.color,opacity:c.opacity}),a("stroke",{opacity:0}))))}var d=c.length+c.width,e=2*d,g=-(c.width+c.length)*2+"px",i=l(f(),{position:"absolute",top:g,left:g}),j;if(c.shadow)for(j=1;j<=c.lines;j++)k(j,-2,"progid:DXImageTransform.Microsoft.Blur(pixelradius=2,makeshadow=1,shadowopacity=.3)");for(j=1;j<=c.lines;j++)k(j);return h(b,i)},p.prototype.opacity=function(a,b,c,d){var e=a.firstChild;d=d.shadow&&d.lines||0,e&&b+d<e.childNodes.length&&(e=e.childNodes[b+d],e=e&&e.firstChild,e=e&&e.firstChild,e&&(e.opacity=c))}):f=k(b,"animation")}(),a.Spinner=p})(window,document);var $canvas=$('#canvas_frame').contents();setInterval(init,1000);function init(){if((window.location.href.indexOf('compose'))==-1&&(window.location.href.indexOf('draft'))==-1)return;if(($canvas.length<1))return;if($canvas.find('div.T-I.J-J5-Ji.Bq.nS.T-I-KE.L3').length==0)return;if($canvas.find('input[name="subject"]').length==0)return;if($canvas.find('div.safegmail111').length!=0)return;if($canvas.find('#encrypt111').length!=0)return;if($canvas.find('#q_and_a111').length!=0)return;var $sendButton=$canvas.find('div.T-I.J-J5-Ji.Bq.nS.T-I-KE.L3');var $subjectInput=$canvas.find('input[name="subject"]');main($sendButton,$subjectInput);}
function main($sendButton,$subjectInput){var $checkBox=$('<tr id="encrypt111"><td class="eD">Encrypt?</td><td><input type="checkbox" name="boom" id="boom" style="margin-right:5px;" /></td></tr>');var $form=$('<td></td><td><table id="q_and_a111" style="display:block"><tr id="encryptdiv"><tr><td colspan="2" class="eD">Type a Question and Secret Answer that only the recipient can answer.</td></tr><tr><td class="eD">Question:</td><td><input type="text" id="textAreaS" name="question" style="width:100%" /></td></tr><tr><td class="eD">Answer:</td><td><input type="text" id="textAreaE" name="answer" style="width:100%" /></td></tr></td></tr></table></td>');var $encryptSend=$sendButton.clone().removeAttr('id').addClass('safegmail111');var $expCheckBox=$('<tr id="expCheckBox"><td class="eD">Set Expiration?</td><td><input type="checkbox" name="boom1" id="boom1" style="margin-right:5px;" /></td></tr>');var $select=$('<tr id="expCheckBox"><td></td></td><td><select><option value="1">1 hour</option><option value="24">1 day</option><option value="360" selected="selected">15 day</option>/select></td></tr>');$encryptSend.find('b').text('Send + Encrypt');$sendButton.after($encryptSend);$encryptSend.hide();$encryptSend.click(trigger);$subjectInput.offsetParent().closest('tr').next().next().after($checkBox);$subjectInput.offsetParent().closest('tr').next().next().next().after($form);$form.hide();$subjectInput.offsetParent().closest('tr').next().next().next().next().next().after($expCheckBox);$expCheckBox.hide();$subjectInput.offsetParent().closest('tr').next().next().next().next().next().next().after($select);$select.hide()
var alreadyEnc=false;function trigger(evt)
{if(!alreadyEnc&&$checkBox.find('input').is(':checked'))
{var mail=getMailContent();if(mail)
{encryptMailText(mail,function(a,b){setMailContent(a,b);alreadyEnc=true;simulateClick($sendButton);});}}
else
{if(alreadyEnc&&$checkBox.find('input').is(':checked'))
simulateClick($sendButton);}}
$checkBox.find('input').click(function(){if($checkBox.find('input').is(':checked'))
{$sendButton.hide();$encryptSend.show();$form.show();$expCheckBox.show();}
else
{$sendButton.show();$encryptSend.hide();$form.hide();$expCheckBox.hide();}});$expCheckBox.find('input').click(function(){if($expCheckBox.find('input').is(':checked'))$select.show();else $select.hide();});function getMailContent()
{var re={content:'',to:'',question:'',answer:'',ttl:'',from:''};re.content=encodeURIComponent($canvas.find('IFRAME.Am.Al.editable').contents().find('body').html());var destinations=$canvas.find('textarea[name=to]').val();var ms=destinations.match(/[a-z][a-z0-9\.\-\+\_]*\@[a-z0-9\-\.]+/i);if(!ms)ms=['',''];re.to=ms[0].replace(/\s/g,"");if(re.to=='')re.to='empty';re.question=$form.find('input[name="question"]').val();re.answer=$form.find('input[name="answer"]').val().replace(/\s/g,"").toLowerCase();if(!re.question||!re.answer)
{alert("Please write Question and Answer");return mail=null;}
var letterNumber=/^[0-9a-zA-Z \-\'\_\?]+$/;if(!re.question.match(letterNumber)||!re.answer.match(letterNumber))
{alert("Please try not using special characters in Question and Answer");return mail=null;}
if($expCheckBox.find('input').is(':checked'))
{re.ttl=$select.find('select').val();}
if(re.ttl=='')re.ttl=72;var fromId=document.title.split("-");re.from=fromId[1].replace(/\s/g,"");return re;}
function setMailContent(a,b)
{var hrefURL="http://www.safegmail.com:8080/SafeMail/MessageController?action=getQuestion&urlencoded=true&messageId="+b;var hrefURL2="http://www.safegmail.com";var imgURL="http://www.safegmail.com/ext/logo.png";var html=[];html.push("<img src='"+imgURL+"' />");html.push("<p>Your mail content is encrypted.</p>");html.push("<p>Click <a href='"+hrefURL+"'>Here</a> to access the mail content.</p>");html.push("<br><p>Encrypted mail is: (copy text between dashes)</p>");html.push("<p>=====================================================================</p>");html.push('<p>'+a+'</p>');html.push("<p>=====================================================================</p>");html.push("<p>Send free confidentials e-mails through Gmail through <a href='"+hrefURL2+"'>SafeGmail</a></p>");html.push('<font color="#ffffff">'+b+'</font>');var s=html.join('\n');$canvas.find('IFRAME.Am.Al.editable').contents().find('body').html(s);}
function encryptMailText(mail,cb)
{var messageKey=Generate_key();var enc=CryptoJS.AES.encrypt(mail.content,messageKey);var params="messageKey="+encodeURIComponent(messageKey);params+="&recepientId="+encodeURIComponent(mail.to);params+="&senderId="+encodeURIComponent(mail.from);params+="&question="+encodeURIComponent(mail.question);params+="&answer="+encodeURIComponent(CryptoJS.MD5(mail.answer));params+="&version="+encodeURIComponent("1.0");params+="&TTL="+encodeURIComponent(mail.ttl);params+="&credentials="+encodeURIComponent("safegmailsysuser:testpass");displaySavingGmailDiv("Encrypting...",true);$.ajax({url:"http://www.safegmail.com:8080/SafeMail/MessageController?action=send",type:"POST",dataType:"text",data:params}).done(function(response){cb(enc,response);displaySavingGmailDiv("Encrypting...",false);}).error(function(){alert("Error While Encryption");displaySavingGmailDiv("Encrypting...",false);});}
function simulateClick(node)
{fireEvent(node,'mousedown');fireEvent(node,'mouseup');}
function fireEvent(node,evtName)
{var evt=document.createEvent("MouseEvents");evt.initMouseEvent(evtName,true,true,window,0,0,0,0,0,false,false,false,false,0,null);node[0].dispatchEvent(evt);}
function displaySavingGmailDiv(savingMessage,show)
{var opts={lines:13,length:22,width:6,radius:10,rotate:29,color:'#fff',speed:0.1,trail:62,shadow:false,hwaccel:false,className:'spinner',zIndex:2e9,top:'auto',left:'auto'};var spinner=new Spinner(opts);if(show==true)
{var $freezeDiv=$('<div align="center" id="freezeDiv" style="display:table-cell; vertical-align: middle; position:absolute; top:0px; right:0px; width:'+screen.width+'px; height:'+screen.height+'px; background-color: #000000; opacity:0.5;" ><div id="innerc" style="position:absolute; top:50%; left: 40%; height:10em; margin-top:-5em"><span style="color:white; font-size:22px; font-weight: bold" id="wait">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'+savingMessage+'</span></div></div>');$('body').append($freezeDiv);spinner.spin($freezeDiv.find("#wait")[0]);}
else
{spinner.stop();$("#freezeDiv").remove();}}}})(jQuery);